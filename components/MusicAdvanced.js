...
class MusicAdvanced extends Component {
      ...
      <View style={[Styles.container_front,{backgroundColor:'#FFFFFF'}]}>
          ...
          <View style={Styles.header_front_music}>
              <View style={Styles.header_view}>
                  <Text  style={Styles.header_title} >Thiền chuyên sâu</Text>
          ...
          <View style={Styles.content_front_music_advanced}>
              ...
              <ScrollView style={Styles.list_3_colum_full} showsVerticalScrollIndicator={false} >
                    {this.state.GridListItems.map((itemData) => {
                        return (
                            <View>
                            <Text style={Styles.header_music_title}>{itemData.category}</Text>
                            <Text style={Styles.header_music_des}>{itemData.category_des}</Text>
                            <FlatList
                                data={ itemData.items }
                                renderItem={ ({item}) =>
                                <TouchableWithoutFeedback onPress={() => {
                                  ...
                                  <View style={Styles.item_3_colum_music}>
                                      ...
                                      <Text style={Styles.content_items_title_music_detail}> {item.name} </Text>
                                      {item.type == "buy" ?
                                      <Icon name='ios-lock' color='#FFFFFF' size={30} style={{position:"absolute",right : 5,bottom:0}} />
                                      : null
                                      }
                                      {item.type == "coming soon" ?
                                      <Icon name='ios-add' color='#FFFFFF' size={30} style={{position:"absolute",right : 5,bottom:0}} />
                                      : null
                                      }
                                  </View>
  ...
  componentDidMount(){
      ...
      fetch('http://thienyoga.net/api/get-advanced-music/7', {
            method: 'GET',
            headers: {
...
export default connect(mapStateToPropsRefesh)(MusicAdvanced);
